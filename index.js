...
const imapConfig = {
  user: 'focograal@gmail.com',
  password: 'jbrc bhde bpip ubfq',
  host: 'imap.gmail.com',
  port: 993,
  tls: true,
  tlsOptions: {
...
imap.once('ready', () => {
console.log('conexão email ligada!');
...
if (searchErr) {
console.error('Error ao procurar mensagens:', searchErr);
return;
}
...
//console.log('pessoa:', headers.get('from'));
//console.log('assunto:', headers.get('subject'));
console.log(headers.get('from').value[0].address)
//if (headers.get('from').includes

});
mailParser.on('data', (data) => {
if (data.type === 'text' && data.text.includes('Someone requested to use your GraalOnline account using a new device or location.')) {
reply(data.text)
}else{
return reply('entre no jogo graal online era e pessa o acesso com o email focograal@gmail.com, logo em seguida volte aqui e der o seguinte comando!: .acessar')
}
});
